...
module Make
  ...
  module API_server = struct
    ...
    let uri_pattern_token = Re2.create_exn "^/api/v1/media/([^/]+)/token/?$"
    ...
    let respond_media_token req id =
      let headers = (Header.init () |> Header.add_list) [
          ("Allow", "GET, OPTIONS");
        ] in
      match Request.meth req with
      | `GET ->
        (let module Store = Application.Store in
         ...
         | Some m ->
           ...
           let r = Token.generate key (Token.Date.advance_by_hours
               (Token.Date.now ()) 2) |> Token.to_string
           in
           Server.respond_string ~headers ~status:`OK ~body:r ())
      | `OPTIONS -> Server.respond ~headers ~status:`OK ~body:Body.empty ()
      | _ ->
        Server.respond_error ~headers ~status:`Method_not_allowed ~body:"" ()
    ...
    let respond req body = function
      ...
      | p when Re2.matches uri_pattern_token p ->
        let matches = Re2.get_matches_exn uri_pattern_token p |> List.hd in
        let id = Re2.Match.get_exn ~sub:(`Index(1)) matches in
        respond_media_token req id
  ...
  module HLS_server = struct
    ...
    let respond req id path =
      ...
      | Some `Other s ->
        ...
        | Ok t when Token.is_valid key t -> (
        ...
        | _ -> Server.respond ~headers ~status:`Forbidden ~body:Body.empty ())
      | _ -> Server.respond ~headers ~status:`Unauthorized ~body:Body.empty ()
  ...
  let server =
    let callback _conn req body =
      ...
      | p when Re2.matches HLS_server.uri_pattern p ->
        let matches = Re2.get_matches_exn HLS_server.uri_pattern p |> List.hd in
        let id = Re2.Match.get_exn ~sub:(`Index(1)) matches in
        HLS_server.respond req id p
      | _ -> Server.respond_error ~status:`Not_found ~body:"" ()
    in
